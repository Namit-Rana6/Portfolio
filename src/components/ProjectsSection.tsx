...
const ProjectCard = ({ project, index, total, containerRef }: ProjectCardProps) => {
  ...
  return (
    <div
      ...
      style={{ top: `${72 + index * 20}px`, height: 'clamp(420px, 80vh, 85vh)' }}
    >
      <motion.article
        style={{ scale }}
        className="origin-top mx-auto h-full w-full flex flex-col gap-3 sm:gap-6 md:gap-8 rounded-[28px] sm:rounded-[50px] md:rounded-[60px] border-2 border-[#D7E2EA] bg-[#0C0C0C] p-3 sm:p-6 md:p-8 cursor-pointer group hover:border-[#D7E2EA]/80 transition-colors"
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:justify-between gap-4 sm:gap-6">
                    <div className="flex flex-row items-start gap-3 sm:gap-6 md:gap-10 min-w-0 w-full">
                      <div
                        ...
                        style={{ fontSize: 'clamp(1.8rem, 8vw, 140px)' }}
                      >
                        {project.number}
                      </div>
                      ...
                      <div className="flex flex-col gap-1 sm:gap-3 pt-1 sm:pt-3 md:pt-4 min-w-0 flex-1">
                        ...
                        {project.status === 'Honorable Mention' && (
                          <span className="inline-block mt-1 sm:mt-2 px-3 py-1 rounded-lg bg-amber-500/20 text-amber-300 text-xs sm:text-sm font-semibold uppercase tracking-wider border border-amber-500/40 w-fit">
                            🏆 Honorable Mention - Google &amp; Kaggle Tunix Hackathon
                          </span>
                        )}
                        {project.status === '1st Place - Vihaan 8.0 Hackathon (IEEE DTU)' && (
                          <span className="inline-block mt-1 sm:mt-2 px-3 py-1 rounded-lg bg-yellow-500/20 text-yellow-300 text-xs sm:text-sm font-semibold uppercase tracking-wider border border-yellow-500/40 w-fit">
                    ...
                    <div className="shrink-0 self-start sm:self-auto pt-1 sm:pt-2 md:pt-3 w-full sm:w-auto flex gap-2 sm:gap-3">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          navigate(`/project/${project.id}`);
                        }}
                        className="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-lg bg-[#D7E2EA]/10 text-[#D7E2EA] hover:bg-[#D7E2EA]/20 border border-[#D7E2EA]/20 transition-all text-xs sm:text-sm font-medium uppercase tracking-wider group/btn"
                      >
                        <span>More Info</span>
                        <ArrowUpRight size={16} className="group-hover/btn:translate-x-0.5 group-hover/btn:-translate-y-0.5 transition-transform" />
                      </button>
                      <LiveProjectButton href={project.liveUrl} className="shrink-0" />
                    </div>
                  </div>
